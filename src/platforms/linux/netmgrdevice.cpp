...
#define DBUS_NM_DEVICE_STATISTICS (DBUS_NM_SERVICE ".Device.Statistics")
...
uint64_t NetmgrDevice::rxBytes() const {
  QDBusInterface stats(DBUS_NM_SERVICE, m_interface.path(),
                       DBUS_NM_DEVICE_STATISTICS,
                       QDBusConnection::systemBus());
  return stats.property("RxBytes").toULongLong();
}

void NetmgrDevice::setStatisticsRefreshRate(uint msec) const {
  QDBusInterface stats(DBUS_NM_SERVICE, m_interface.path(),
                       DBUS_NM_DEVICE_STATISTICS,
                       QDBusConnection::systemBus());
  if (!stats.setProperty("RefreshRateMs", msec)) {
    logger.warning() << "Failed to set NetworkManager statistics refresh rate";
  }
}
