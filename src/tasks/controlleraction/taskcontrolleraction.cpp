...
bool activateLocalProfile(Controller* controller,
                          Controller::ActivationPrincipal principal) {
  InterfaceConfig config;
  QString error;
  if (!MozillaVPN::instance()
           ->wireGuardProfileModel()
           ->activeInterfaceConfig(config, &error)) {
    logger.error() << "Unable to activate profile:" << error;
    return false;
  }

  return controller->activate(config, principal);
}
}
...
void TaskControllerAction::run() {
  ...
  switch (m_action) {
    case eActivate:
      if (MozillaVPN::instance()->wireGuardProfileModel()->hasProfiles()) {
        expectSignal = activateLocalProfile(controller, Controller::ClientUser);
      } else {
    ...
    case eActivateForExtension:
      if (MozillaVPN::instance()->wireGuardProfileModel()->hasProfiles()) {
        expectSignal =
            activateLocalProfile(controller, Controller::ExtensionUser);
      } else {
        expectSignal =
    ...
    case eSilentSwitch:
      if (MozillaVPN::instance()->wireGuardProfileModel()->hasProfiles()) {
        logger.debug()
            << "Silent server switch skipped for local WireGuard profile";
        expectSignal = false;
      } else {
    ...
    case eSwitch:
      if (MozillaVPN::instance()->wireGuardProfileModel()->hasProfiles()) {
        logger.debug() << "Server switch skipped for local WireGuard profile";
        expectSignal = false;
      } else {
...
void TaskControllerAction::stateChanged() {
  ...
  if (((m_action == eActivate || m_action == eSwitch) &&
      ...
      (m_action == eActivateForExtension &&
