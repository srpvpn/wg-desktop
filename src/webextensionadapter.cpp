      ...
      RequestType{"servers",
                  [this](const QJsonObject&) {
                    ...
                    if (!vpn->wireGuardProfileModel()->hasProfiles()) {
                      serializeServerCountry(vpn->serverCountryModel(),
                                             servers);
                    }
...
QJsonObject WebExtensionAdapter::serializeStatus() {
  ...
  if (profiles->hasProfiles()) {
    locationObj["active_profile_id"] = profiles->activeProfileId();
    locationObj["active_profile_name"] = profiles->activeProfileName();
  } else {
