...
void Controller::initialize() {
  ...
  const Keys* keys = nullptr;
  if (!vpn->wireGuardProfileModel()->hasProfiles()) {
    device = vpn->deviceModel()->currentDevice(vpn->keys());
    keys = vpn->keys();
  }
...
void Controller::handshakeTimeout() {
  ...
  if (vpn->wireGuardProfileModel()->hasProfiles()) {
    logger.error() << "Local WireGuard profile handshake timed out";
...
void Controller::activateNext() {
  ...
  if (m_activationQueue.isEmpty()) {
    logger.error() << "No WireGuard configuration queued for activation";
    setError(ErrorFatal);
    setState(StateConnectionError);
    return;
  }
...
bool Controller::activate(const ServerData& serverData,
  ...
  if (m_state == Controller::StateOff) {
    ...
    logger.debug()
        << "Pre-activation account check skipped in standalone mode";
...
bool Controller::activate(const InterfaceConfig& config,
                          ActivationPrincipal initiator) {
  logger.debug() << "Local profile activation" << m_state;
  if (m_state != Controller::StateOff &&
      m_state != Controller::StateOnPartial &&
      m_state != Controller::StateSwitching &&
      m_state != Controller::StateSilentSwitching &&
  ...
  if (!m_impl || !m_backendAvailable) {
    logger.warning() << "VPN backend is unavailable. Aborting activation.";
    setError(ErrorNoServerAvailable);
    setState(StateConnectionError);
    return false;
  }
  ...
  if (m_state == Controller::StateOff &&
      Feature::isEnabled(Feature::checkConnectivityOnActivation)) {
    if (MozillaVPN::instance()->networkWatcher()->getReachability() ==
        QNetworkInformation::Reachability::Disconnected) {
      logger.debug() << "Device has no network connectivity.";
      m_isDeviceConnected = false;
      emit isDeviceConnectedChanged();
      return false;
    }
  ...
  InterfaceConfig localConfig = config;
  if (m_impl->splitTunnelSupported()) {
    localConfig.m_vpnDisabledApps =
  ...
  m_handshakeTimer.stop();
  m_activationQueue.clear();
  m_activationQueue.append(localConfig);

  m_pingReceived = false;
  const auto& firstConfig = m_activationQueue.first();
  if (!firstConfig.m_serverIpv4AddrIn.isEmpty()) {
    m_pingCanary.start(firstConfig.m_serverIpv4AddrIn, "0.0.0.0/0");
  } else if (!firstConfig.m_serverIpv6AddrIn.isEmpty()) {
    m_pingCanary.start(firstConfig.m_serverIpv6AddrIn, "::/0");
  }
